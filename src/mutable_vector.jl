...
"""
mutable struct MVectorFold{T,V <: AbstractVector{T}} <: PatternFold{T,V}
    pattern::V
    gap::T
    folds::Int
    current::Int
...
MVectorFold(p, g, f; c = 1) = MVectorFold(p, g, f, c)

pattern_length(mvf::MVectorFold) = length(mvf.pattern)

function pattern(mvf::MVectorFold)
...
function Base.iterate(iter::MVectorFold)
    reset_pattern!(iter)
    return pattern(iter, 1), 1
end

# "Induction" iterate method
function Base.iterate(iter::MVectorFold, state::Int)
...
function Base.iterate(r_iter::Base.Iterators.Reverse{MVectorFold{T,V}},
